...
function extractExperience($) {
    const EXP_PATTERNS = [
        ...
        // Vietnamese: "3 năm kinh nghiệm", "từ 2 năm", "≥ 2 năm kinh nghiệm"
        ...
        // "3 - 5 năm"
        /(\d+)\s*[-–]\s*(\d+)\s*năm/i,
        // FIX BUG-7: "≥ 2 năm" / ">= 3 years" — KHÔNG yêu cầu chữ "kinh
        // nghiệm" hoặc "experience" theo ngay sau, vì câu có thể viết
        // "≥ 2 năm [mô tả kỹ năng khác]" rồi mới tới câu khác hẳn.
    ...
    searchScope.find(CONTENT_TAGS.join(', ')).each((_, el) => {
        ...
        const text = $(el).text().trim();
        // FIX BUG-8: KHÔNG còn loại bỏ cả block khi dài hơn 300 ký tự.
        // Requirements thường được viết liền thành một đoạn dài (không
        // xuống dòng giữa các câu), nên block chứa câu kinh nghiệm hợp lệ
        // vẫn có thể dài hơn 300 ký tự — loại bỏ cả block làm mất luôn câu
        // kinh nghiệm nằm trong đó. Giữ giới hạn 5000 ký tự chỉ để chặn
        // trường hợp $(el).text() vô tình bắt cả document (an toàn, không
        // ảnh hưởng câu thật).
        ...
        for (const pat of EXP_PATTERNS) {
            ...
            if (m) {
                // FIX BUG-8: cắt context XUNG QUANH vị trí match thay vì
                // luôn lấy 150 ký tự đầu của block. Với block dài, câu
                // kinh nghiệm có thể nằm ở giữa hoặc cuối — lấy từ đầu sẽ
                // bị cắt mất nội dung thật, không phải lỗi crash nhưng làm
                // output kém hữu ích.
