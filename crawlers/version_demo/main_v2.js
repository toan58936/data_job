...
const CONFIG = {
    ...
    outputDir: './output',
    outputFile: './output/jobs_detail.json',
    checkpointFile: './output/checkpoint_detail.json',
    maxRetries: 2,
    retryDelay: 5000,
    delayBetweenJobs: 7000,

    // ✅ FIX: Chạy ẩn hoàn toàn — không hiện trình duyệt
    headless: true,

    // ✅ FIX: Thêm args giả lập môi trường thật khi headless
    // Tránh TopCV detect headless qua navigator.webdriver hoặc thiếu GPU flag
    launchArgs: [
        '--no-sandbox',
...
function upsertJobWithDeadlineCheck(newJob, existingJob) {
    if (newJob.deadline !== existingJob.deadline) {
        log(`Deadline changed for ${newJob.title} (${existingJob.deadline} -> ${newJob.deadline}), updating...`, 'INFO');
        return true;
    }
    return true;
}
...
async function crawlJobDetail(page, jobUrl, retryCount = 0) {
    ...
    try {
        ...
        // ✅ FIX: Tăng wait khi headless — Vue cần thêm thời gian hydrate
        // Headless thiếu GPU nên JS render chậm hơn ~500ms so với có UI
...
(async () => {
    ensureDir(CONFIG.outputDir);
    log('=== START CRAWL ALL JOB DETAILS ===');
    ...
    const jobList = JSON.parse(fs.readFileSync(CONFIG.inputFile, 'utf8'));
    log(`Loaded ${jobList.length} jobs from ${CONFIG.inputFile}`);
    ...
    let startIndex = 0;
    if (checkpoint && checkpoint.totalJobs === jobList.length) {
        startIndex = checkpoint.lastIndex + 1;
        log(`Resume from checkpoint: index ${startIndex}/${jobList.length}`);
    } else {
        if (checkpoint) log('Checkpoint ignored because total jobs changed.', 'WARN');
        log('Starting fresh crawl');
        if (fs.existsSync(CONFIG.outputFile)) fs.unlinkSync(CONFIG.outputFile);
    }

    // ✅ FIX: Truyền launchArgs vào chromium.launch
    ...
    // ✅ FIX: Ẩn dấu hiệu automation trong headless mode
    // navigator.webdriver = true là dấu hiệu dễ bị site detect nhất
    ...
    try {
        for (let i = startIndex; i < jobList.length; i++) {
            const job = jobList[i];
            log(`Progress: ${i+1}/${jobList.length} - ${job.title} (${job.company})`);
            ...
            saveCheckpoint(i, jobList.length, job.url);
        }
    } finally {
        await browser.close();
        log('Browser closed.');
